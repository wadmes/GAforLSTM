...
def mutation(parent):
    ...
    current_ix = list(conf.gene_base[mut_ix]).index(parent[mut_ix])
    if current_ix == 0:
        new_ix = current_ix + 1
    elif current_ix == conf.gene_base[mut_ix].shape[0] - 1:
        new_ix = current_ix - 1
    else:
        if random.uniform(0,1) < 0.5:
            ...
            new_ix = current_ix - 1
    parent[mut_ix] = conf.gene_base[mut_ix][new_ix]
    return parent
...
def GP(gene_num=conf.gene_num, generation_num=conf.generation_num,
    ...
    for g in range(generation_num):
        ...
        next_generation = []
        next_generation.append(population[fitness.index(max(fitness))])
        while len(next_generation) < population_size:
            if population_size - len(next_generation) < 2:
                rand = random.uniform(pc, pc + pr + pm)
            else:
                rand = random.uniform(0, pc + pr + pm)
    ...
    # Evaluation (Compare with baseline models)
    ...
    with open('../results/best_individual.txt','w') as file:
        file.write('GP_fitness: {}'.format(GP_fitness))
    print 'Write simulation process into csv file...'
    write_to_csv(list(test_date)[best_individual[4]:], capital_simulate, capital_stock, '../results/best_individual.csv')
    # Average individual
    ...
    with open('../results/avg_individual.txt','w') as file:
        file.write('avg_fitness: {}'.format(avg_fitness))
    print 'Write simulation process into csv file...'
    write_to_csv(list(test_date)[avg_individual[4]:], capital_simulate, capital_stock, '../results/avg_individual.csv')
    # Random guess
    print 'Evaluate random guess...'
    ...
    with open('../results/random_guess.txt','w') as file:
        file.write('random_guess_fitness: {}'.format(random_guess_fitness))
    print 'Write simulation process into csv file...'
    write_to_csv(list(test_date)[1:], capital_simulate, capital_stock, '../results/random_guess.csv')
    # Sentiment only
    print 'Evaluate sentiment only...'
    capital_simulate, capital_stock = simulation.sentiment_only(sentiment,list(test['adj_close']))
    sentiment_only_fitness = fitness_measure(capital_simulate, capital_stock)
    print 'sentiment_only_fitness: {}'.format(sentiment_only_fitness)
    with open('../results/sentiment_only.txt','w') as file:
